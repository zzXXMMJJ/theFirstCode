...
cc.Class({
	...
	start() {
		...
		this.startY = 175 // this.node.y;
		// console.log('sta' + this.startY, this.node.y, gameManager.curScene.titleIndex)

		this.loadData(gameManager.showSingleHash[gameManager.curScene.titleIndex]);

		gameManager.curScene.checkTileFlag = true;
	},
	// 加载列表 需要加还原
	loadData: function (list) {
		...
		this.maxDex = Math.floor((list.length - 1 + flag) / this.colNum) * this.colNum;

		for (let i = this.curDex; i < this.curDex + this.colNum * this.maxRow && i < list.length + flag; i++) {
			this.initItemFun(i, childs[i - this.curDex]); //生成当前界面可以生成的最大数量
		}

		if (list.length + flag - this.curDex >= this.colNum * (this.maxRow - 1)) //所要生成的节点数占据到最大限制的最后一行
		{
			if (list.length + flag - this.curDex > this.colNum * this.maxRow) {
				this.isScrollChange = true; //所要生成的节点数>最大限制数量   确认可以滚动
			}
			this.bottomIndex = this.colNum * (this.maxRow - 1); //最后面那个节点对应在父节点中的位置---限制的最大行的第一位索引
		}

		let row = Math.ceil((list.length + flag) / this.colNum); //当期需要加载的最大行数
	...
	initItemFun: function (index, child) {
		if (index > this.itemlist.length - 1 + this.refreshFlag) {
		...
		if (child != null || child != undefined) {
			...
			obj = cc.instantiate(this.itemPrefab);
			obj.parent = this.node;
		}

		obj.active = true;

		let item = obj.getComponent("itemsmallClick");
		item.index = index;

		if (this.refreshFlag) {
			if (index == 0) {
				item.initIconFun(index, this.itemlist[index], this.itemMsgList, this.refreshFlag);
			} else {
				item.initIconFun(index, this.itemlist[index - 1], this.itemMsgList, this.refreshFlag);
			}

		}
		let yy = -this.itemPadding - this.itemH / 2 - Math.floor(index / this.colNum) * (this.itemH + this.itemSpace);
		let xx = -this.node.width / 2 + this.itemPadding + this.itemW / 2 + index % this.colNum * (this.itemW + this.itemSpace);
	...
	UpdateTwoRowItem(flag) {
		...
		if (flag == 0) { //把最上面的那排移动到最下面
			let imageIndex = childs[this.topIndex].getComponent("itemsmallClick").index + this.colNum * this.maxRow;
			if (imageIndex >= this.itemlist.length + (this.refreshFlag)) //不进行移动 已经是最底下了
				return;
			...
			let imageIndex = childs[this.bottomIndex].getComponent("itemsmallClick").index - this.colNum * this.maxRow;
			if (imageIndex < 0)
				return;
